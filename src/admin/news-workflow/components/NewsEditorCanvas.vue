...
import type { NewsEditorBlock } from '@/admin/news-workflow/types/newsWorkflow'

const store = useNewsWorkflowEditorStore()

type PointerMode = 'drag' | 'resize'
const pointerState = {
  mode: null as PointerMode | null,
  blockId: '',
  startClientX: 0,
  startClientY: 0,
  originX: 0,
...
function startDrag(event: MouseEvent, block: NewsEditorBlock) {
...
function startResize(event: MouseEvent, block: NewsEditorBlock) {
  event.preventDefault()
  event.stopPropagation()
...
function updateEditableContent(block: NewsEditorBlock, html: string) {
  store.updateBlockContent(block.id, html, { recordHistory: false })
}

function flushEditableContent(block: NewsEditorBlock, html: string) {
  store.updateBlockContent(block.id, html, { recordHistory: true })
}

function updateTwoColumn(block: NewsEditorBlock, key: 'leftContent' | 'rightContent', html: string, commit = false) {
...
function selectGalleryItem(event: MouseEvent, blockId: string, itemId: string) {
  event.stopPropagation()
  store.selectBlock(blockId)
  store.selectSubItem(itemId)
}

function removeGalleryItem(event: MouseEvent, block: NewsEditorBlock, itemId: string) {
  event.stopPropagation()
  const images = Array.isArray(block.props?.images) ? [...block.props.images] : []
  const nextImages = images.filter((item: any) => String(item?.id || '') !== itemId)
...
function getImageCaption(block: NewsEditorBlock): string {
      ...
      >
        ...
        <template v-if="block.type === 'text' || block.type === 'heading' || block.type === 'quote'">
          <div
            ...
            @input="updateEditableContent(block, ($event.target as HTMLDivElement).innerHTML)"
            @blur="flushEditableContent(block, ($event.target as HTMLDivElement).innerHTML)"
          />
        </template>
        ...
        <template v-else-if="block.type === 'two_column'">
          <div class="two-column-block" :style="{ gap: `${Number(block.props?.gap || 24)}px` }">
            <div
              ...
              @input="updateTwoColumn(block, 'leftContent', ($event.target as HTMLDivElement).innerHTML, false)"
              @blur="updateTwoColumn(block, 'leftContent', ($event.target as HTMLDivElement).innerHTML, true)"
            />
            <div
              class="editable-column"
              contenteditable="true"
              ...
              @input="updateTwoColumn(block, 'rightContent', ($event.target as HTMLDivElement).innerHTML, false)"
              @blur="updateTwoColumn(block, 'rightContent', ($event.target as HTMLDivElement).innerHTML, true)"
            />
          </div>
        </template>
