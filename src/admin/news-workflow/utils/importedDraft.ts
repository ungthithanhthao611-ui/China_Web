import type { NewsContentJson, NewsEditorBlock, NewsPostModel } from '@/admin/news-workflow/types/newsWorkflow'
...
function makeBlockId(prefix: string, index: number): string {
  return `${prefix}-${Date.now()}-${index}`
}

function pushBlock(
  blocks: NewsEditorBlock[],
  next: Omit<NewsEditorBlock, 'id' | 'x' | 'y'>
) {
...
function pushTextBlock(blocks: NewsEditorBlock[], content: string, height = 140) {
  pushBlock(blocks, {
    type: 'text',
    w: 820,
    h: height,
...
function walkNode(node: ChildNode, blocks: NewsEditorBlock[]) {
...
function createFallbackBlocks(title: string, summary: string, html: string): NewsEditorBlock[] {
  const blocks: NewsEditorBlock[] = []
...
export function buildContentJsonFromImportedHtml(title: string, summary: string, html: string): NewsContentJson {
  const blocks: NewsEditorBlock[] = []
...
export function persistImportedDraft(preview: any) {
  ...
  const payload: Partial<NewsPostModel> = {
    title,
...
export function consumeImportedDraft(): Partial<NewsPostModel> | null {
  try {
