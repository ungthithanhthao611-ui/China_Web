...
import { deleteAdminNews, importWorkflowFile, listAdminNews, listNewsCategories } from '@/admin/api/newsWorkflowApi.js'
import { persistImportedDraft } from '@/admin/utils/news-workflow/importedDraft.js'
import { consumeNewsWorkflowFlash } from '@/admin/utils/news-workflow/newsWorkflowFlash.js'
...
function formatCategories(item) {
  ...
  return categories.length ? categories.map((category) => category?.name).filter(Boolean).join(', ') : '--'
}
